...
<xsl:template match="/">
<plain-text-result> <!-- This wrapper element was added so that we have an XML result -->
...
<xsl:apply-templates select="*"/> <!-- removed predicate [not(self::title)] -->
</xsl:template>
...
<xsl:template match="footnote">//footnote[<xsl:value-of select="@backrefs" />][<xsl:apply-templates select="*"/>]<xsl:text> <!-- removed predicate [not(self::label)] -->
